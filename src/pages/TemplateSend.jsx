...
import { alpha, Autocomplete, Box, Button, Card, CardContent, CardActions, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Fade, ListItemIcon, ListItemText, Menu, MenuItem, Stack, TextField, Typography, styled } from '@mui/material';
...
const fetchTemplates = async (appId, authCode) => {
  try {
    ...
    if (data.status === 'success') {
      return data.templates.filter(template => template.status === 'PENDING');
    }
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        <Box sx={{ flexGrow: 1, p: 3 }}>
          {/*TITULO*/}<Typography variant="h4" gutterBottom>
            Catálogo de Plantillas Enviadas Pendientes
          </Typography>
